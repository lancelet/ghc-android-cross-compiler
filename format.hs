...
{-
FORMATTING UTILITY
This script runs both Ormolu and "cabal format" as formatting stages that can
be used to both:
  ...
  unsafePerformIO $
    Path.IO.withSystemTempFile ".cabal" $
      \filePath handle -> do
        ...
        let cp = Process.proc "cabal" ["format", Path.fromAbsFile filePath]
